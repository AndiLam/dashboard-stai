...
@section('content')
    <form action="{{ $loginRoute }}" method="POST">
        ...
        <div class="mb-4">
            <input id="nuptk" type="text" name="nuptk" placeholder="NUPTK" class="block w-full px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
        </div>
        ...
        <div class="text-center">
            <a href="{{ route('showRole') }}" class="inline-block w-full bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-lg hover:bg-custom-blue-dark transition duration-300">Kembali</a>
        </div>
    </form>
@endsection
